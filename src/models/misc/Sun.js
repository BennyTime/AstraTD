...
export function createSun() {
  ...
  // Convenience: mark a material as unaffected by scene fog
  ...
  // Caller must:  sc.add(sun.light); sc.add(sun.light.target);
  //               sun.light.position.copy(sun.mesh.position);
  // Target defaults to (0,0,0) — aimed at the board centre.
  const light = new THREE.DirectionalLight(0xfff2d0, 2.8);
  light.castShadow = true;
  light.shadow.mapSize.width  = 2048;
  light.shadow.mapSize.height = 2048;
  light.shadow.camera.near   =   1;
  light.shadow.camera.far    = 230;   // must reach origin from ~164 units away
